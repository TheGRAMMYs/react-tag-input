...
class ReactTags extends Component {
  ...
  filteredSuggestions = async (query) => {
    ...
    if (this.props.handleFilterSuggestions) {
      return await this.props.handleFilterSuggestions(query, suggestions);
    }
  ...
  updateSuggestions = async () => {
    ...
    const suggestions = await this.filteredSuggestions(query);
  ...
  handleKeyDown(e) {
    ...
    if (e.keyCode === this.props.autoAddWith) {
      e.preventDefault();
      const suggestedTag = this.state.suggestions?.[0]
  ...
  addTag = async (tag) => {
    ...
    if (this.props.autocomplete) {
      const possibleMatches = await this.filteredSuggestions(tag[labelField]);
